    ...
    React.createElement("div", {
      ...
      insights.slice(0, 3).map((ins, i) =>
        React.createElement("div", {
          ...
          className: "float-up",
          style: {
            display: "flex", gap: 12, alignItems: "flex-start", marginBottom: i < insights.slice(0, 3).length - 1 ? 12 : 0,
...
/* ═══════════════════════════════════════════════════
   INDEXED DATABASE HELPER FOR FILE HANDLES
═══════════════════════════════════════════════════ */
const IDB_NAME = "ML_Tracker_IDB";
...
async function idbStoreHandle(key, handle) {
  try {
...
function FilesTab() {
  ...
  useEffect(() => {
    idbGetHandle("root_dir").then(async handle => {
      if (handle) {
  ...
  useEffect(() => {
    if (!currentHandle || needsPermission) { setFiles([]); return; }
    setLoading(true);
    async function load() {
      try {
        const result = [];
        for await (const entry of currentHandle.values()) {
          result.push({ name: entry.name, kind: entry.kind, handle: entry });
        }
  ...
  async function selectFolder() {
    try {
      ...
      await idbStoreHandle("root_dir", dirHandle);
  ...
  async function navigateTo(folderHandle) {
  ...
  function goBack(index) {
    ...
    setPath(newPath);
    setCurrentHandle(newPath[newPath.length - 1]);
  }
  ...
  return React.createElement("div", null,
    ...
    React.createElement("div", { style: { background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, padding: 16, marginBottom: 20 } },
      React.createElement("p", { style: { fontSize: 13, color: "var(--muted)", marginBottom: 12, lineHeight: 1.5 } }, "Link a folder from your PC to view its contents securely. Due to browser security rules, files opened from the web will preview in a new tab or trigger a download rather than launching your local app directly."),
      React.createElement("div", { style: { display: "flex", gap: 10 } },
        React.createElement("button", {
          onClick: selectFolder,
          style: { flex: 1, background: "#22d3ee0d", border: "1.5px dashed #22d3ee44", borderRadius: 14, padding: 18, color: "#22d3ee", fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }
        }, rootHandle ? "📁 Change Linked Folder" : "📁 Link Local Folder"),
        rootHandle && React.createElement("button", {
          onClick: () => idbStoreHandle("root_dir", null).then(() => { setRootHandle(null); setCurrentHandle(null); setPath([]); }),
          style: { background: "transparent", color: "var(--muted)", border: "1px solid var(--border)", padding: "10px 18px", borderRadius: 14, fontSize: 13, fontWeight: 600 }
        }, "Unlink")
      )
    ),
    rootHandle && React.createElement("div", { style: { background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, padding: 16 } },
      needsPermission ? React.createElement("div", { style: { textAlign: "center", padding: "20px" } },
        React.createElement("div", { style: { marginBottom: 14, color: "var(--text)", fontSize: 13 } }, `Please grant permission to restore session access to "${rootHandle.name}".`),
        React.createElement("button", { onClick: requestPerm, style: { background: "#22d3ee18", border: "1px solid #22d3ee", borderRadius: 8, padding: "8px 16px", color: "#22d3ee", fontWeight: 600 } }, "Grant Access")
      ) : React.createElement(React.Fragment, null,
        React.createElement("div", { style: { fontFamily: "'Syne',sans-serif", fontSize: 15, fontWeight: 800, color: "var(--text)", marginBottom: 14, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" } },
          path.map((p, i) => React.createElement(React.Fragment, { key: i },
            React.createElement("span", {
              onClick: () => goBack(i),
              style: { cursor: i < path.length - 1 ? "pointer" : "default", color: i === path.length - 1 ? "var(--text)" : "var(--muted)", transition: "color 0.2s" }
            }, i === 0 ? "📂 " + p.name : p.name),
            i < path.length - 1 && React.createElement("span", { style: { color: "var(--muted)" } }, "/")
          ))
        ),
          ...
          files.length === 0 ? React.createElement("div", { style: { fontSize: 13, color: "var(--muted)", padding: "10px 0" } }, "Folder is empty") :
            React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 4 } },
              files.map(f => React.createElement("div", {
                key: f.name,
                onClick: () => f.kind === 'directory' ? navigateTo(f.handle) : openFile(f.handle),
                className: "hover-row",
                style: { padding: "8px 10px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 8, display: "flex", alignItems: "center", gap: 10, fontSize: 13, color: "var(--text)", cursor: "pointer" }
              },
...
function CustomizeTab({ data, updSchema, weeklyLog, milestoneStatus, resourceStatus }) {
  ...
  function PhasesSection() {
    ...
    function saveNew() {
      if (!form.id.trim() || !form.label.trim()) return;
      savePhases([...phases, { ...form, target: +form.target || 60 }]);
      setAddMode(false); setForm(blank);
    }
    function del(id) {
      if (!confirm(`Delete ${id}? This will not delete your logged data.`)) return;
  ...
  function MilestonesSection() {
    ...
    function delMile(id) {
      ...
      saveMiles(miles.filter(m => m.id !== id));
    }
...
function App() {
  ...
  /* Update schema and keep status maps in sync with new items */
  const updSchema = newSchema => {
    applySchema(newSchema);
    const d = { ...data, schema: newSchema };
    // Ensure weeklyLog has entries for all weeks
    if (newSchema.weeklyPlan?.length) {
      const wl = { ...d.weeklyLog };
      newSchema.weeklyPlan.forEach(w => { if (!wl[w.w]) wl[w.w] = { hours: "", status: "Not Started", notes: "" }; });
      d.weeklyLog = wl;
    }
    // Ensure milestone status entries
    if (newSchema.milestones?.length) {
      const ms = { ...d.milestones };
      newSchema.milestones.forEach(m => { if (!ms[m.id]) ms[m.id] = { done: false, notes: "" }; });
      d.milestones = ms;
    }
    // Ensure resource status entries
    if (newSchema.resources?.length) {
      const rs = { ...d.resources };
      newSchema.resources.forEach(r => { if (!rs[r.id]) rs[r.id] = { status: "Not Started", notes: "" }; });
      d.resources = rs;
    }
    // Ensure project status entries
    if (newSchema.projects?.length) {
      const ps = { ...d.projects };
      newSchema.projects.forEach(p => { if (!ps[p.id]) ps[p.id] = { status: "Not Started", startDate: "", githubLink: "", linkedIn: false, notes: "", tools: p.tools || "", name: p.name }; });
      d.projects = ps;
    }
    save(d);
  };
